# Wheel of fortune game between computer and human


import sys
sys.setExecutionLimit(600000) # maximum 10 minutes
...
#create player class for human and computer
class WOFPlayer:    
    prizeMoney = 0
    prizes = []
...
#inherit player class to create human player
class WOFHumanPlayer(WOFPlayer):
  ...
  #inherit player class to create computer player     
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        possible_list = []
        reversed_let = self.SORTED_FREQUENCIES[::-1]
        
        for chr in reversed_let:
            if chr not in guessed:
                if (chr in VOWELS) and (self.prizeMoney >= VOWEL_COST):
                    possible_list.append(chr)
                elif chr not in VOWELS:
                    possible_list.append(chr)                    
...
# Repeatedly ask the user for a number between min & max (inclusive)
def getNumberBetween(prompt, min, max):
    ...
    while True:
        ...
        # If no number yet, add the error message
        # and ask again
        userinp = input('{}\n{}'.format(errmessage, prompt))

# Spin the wheel of fortune wheel to give a random prize
# Examples:
...
# Return a category & phrase (as a tuple) to guess
# Example:
...
def requestPlayerMove(player, category, guessed):
    while True: # we're going to keep asking the player for a move until they give a valid one
        time.sleep(1.0) # added so that any feedback is printed out before the next prompt
